        ...
        "highlight": function (options) {
                ...
                return this.each(function () {
                    ...
                    function animation() {
                        ...
                        $this.find(liBox).eq(Math.floor(opts.showNum/2)+s).addClass('ytl_active').siblings().removeClass('ytl_active');
                        $this.find(liBox).css({"margin-top":0});
                        $this.find(liActiveBox).css({"margin-top":-(afterLiH-beforeLiH)/2});
                        if(s>$this.find(liBox).length-opts.showNum){
                            $this.find(listBox).css({"margin-left":-10});
                            $this.find(liBox).eq(Math.floor(opts.showNum/2)).addClass('ytl_active').siblings().removeClass('ytl_active');
                            $this.find(liBox).eq(Math.floor(opts.showNum/2)).removeClass('tstAll');
                            $this.find(liActiveBox).css({"margin-top":-(afterLiH-beforeLiH)/2});
                        }
                        else {
                            $this.find(liBox).addClass('tstAll');
                            var move=parseInt($this.find(listBox).css('margin-left'))-$this.find(liBox).eq(s-1).outerWidth(true);
                            $this.find(listBox).animate({"margin-left":move},500);
                        }
                    }
                    time=setInterval(function () {
                        animation();
