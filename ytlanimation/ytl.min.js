        ...
        "lilyWindow": function (options) {
            ...
            return this.each(function () {
                ...
                $this.find(".ytl_prev").click(function () {
                    ...
                    else if(afterIndex<2&&afterIndex>=0){
...
/*/!*allWidth*!/
function allWidth(div,ul,li) {
    for(var i=0;i<$(div).length;i++){
        var divW = $(ul).eq(i).find(li).length * ($(ul).eq(i).find(li).outerWidth(true));
        $(ul).eq(i).width(divW);
    }
}
/!*show and addClass*!/
function showOn(div,on,ul,moveNum,allW,point) {
    $(div).find('.right').hide();
    $(div).find('.right').eq(0).show();
    $(div).find(on).eq(0).addClass('on');
    $(div).find(".left li").click(function () {
        var num = $(this).index();
        $(div).find('.right').hide();
        $(div).find('.right').eq(num).show();
        $(on).find('ul').css({'margin-left':0});
        $(this).addClass('on').siblings().removeClass('on');
        $(div).find(on).eq(num).addClass('on').siblings().removeClass('on');
        $(div).find('.point li').eq(0).addClass('active').siblings().removeClass('active');
        if(allW){
            for(var i=0;i<$(div).find('.on').length;i++){
                var divW = $(div).find('.on').find(ul).find('li').length * ($(div).find('.on').find(ul).find('li').outerWidth(true));
                $(div).find('.on').find(ul).eq(i).width(divW);
            }
        }
        if(point){
            var pointList='';
            for(var i=0;i<$(div).find('.on').find(ul).find('li').length/moveNum;i++){
                if(i==0){
                    pointList+='<li class="active"></li>';
                }
                else {
                    pointList+='<li></li>';
                }
            }
            $(div).find('.on').find('.point').html(pointList);
            $(div).find('.point li').click(function () {
                $(this).addClass('active').siblings().removeClass('active');
                var move=$(this).index()*$(div).find('.on').find(ul).find('li').outerWidth(true)*moveNum;
                $(div).find('.on').find(ul).stop(true,true).animate({'margin-left': -move + 'px'});
            });
        }
    });
}
/!*play*!/
function lrPlay(autoplay,prevNext,point,div,ul,li,hover,mr,showNum,timer,moveNum) {
    var autoPlay=null;
    var move=0;
    function nextTime() {
        var ulMl=parseInt($(div).find(ul).css('margin-left'));
        var s=-ulMl/($(div).find(li).outerWidth(true)*moveNum);
        if($(div).find(li).length>showNum){
            $(div).find('.point li').eq(s+1).addClass('active').siblings().removeClass('active');
            if (s>($(div).find(li).length-showNum-1)/moveNum) {
                move=0;
                $(div).find('.point li').eq(0).addClass('active').siblings().removeClass('active');
                $(div).find(ul).stop(true,true).animate({'margin-left': 0});
            }
            else{
                move =-ulMl+($(div).find(li).outerWidth(true)*moveNum);
                $(div).find(ul).stop(true,true).animate({'margin-left': -move + 'px'});
            }
        }
    }
    function prevTime() {
        ...
        if($(div).find(li).length>showNum){
            if (s<=0) {
                move =-($(div).find(li).outerWidth(true))*($(div).find(li).length-showNum);
                $(div).find(ul).stop(true,true).animate({'margin-left': move + 'px'});
            }
            else{
                move =ulMl+($(div).find(li).outerWidth(true)*moveNum);
    ...
    if(prevNext){
        $(div).find('.prevNext').hover(function () {
            clearInterval(autoPlay);
        },function(){
            auto();
        });
    ...
    if(point){
        var pointList='';
        for(var i=0;i<$(div).find(li).length/moveNum;i++){
            if(i==0){
                pointList+='<li class="active"></li>';
            }
            else {
        ...
        $(div).find('.point').html(pointList);
        $(div).find('.point').hover(function () {
            clearInterval(autoPlay);
        },function(){
            auto();
        });
        $(div).find('.point li').click(function () {
            $(this).addClass('active').siblings().removeClass('active');
            var move=$(this).index()*$(div).find(li).outerWidth(true)*moveNum;
            $(div).find(ul).stop(true,true).animate({'margin-left': -move + 'px'});
        });
    }
}*/
