    ...
    $.fn.extend({
        ...
        "multipleRolling": function (options) {
            ...
            return this.each(function () {
                ...
                initialize(0);
                isCreatePoint(opts.createPoint, $this);
                $this.width($this.find(ulLiBox).outerWidth(true)*opts.showNum-parseInt($this.find(ulLiBox).css("margin-right")));
                ...
                function animation() {
                    ...
                    var mL=parseInt($this.find(ulBox).eq(num).css("margin-left")),
                        liW=$this.find(ulBox).eq(num).find('li').outerWidth(true),
                        index=$this.find(olBox).find('.ytl_pointActive').index()+1;
                    if(-mL/liW>$this.find(ulBox).eq(num).find('li').length-opts.showNum-1){
                        mL=liW;
                        index=0;
                    }
                    $this.find(olLiBox).removeClass('ytl_pointActive');
                    $this.find(olLiBox).eq(index).addClass('ytl_pointActive');
                    $this.find(ulBox).eq(num).stop(true,true).animate({"margin-left":mL-liW},opts.timer/3);
                }
                if(opts.autoPlay){
                    time=setInterval(function () {
                        animation();
                ...
                function initialize(num) {
                    ...
                    var divW = $this.find(ulBox).eq(num).find('li').length * ($this.find(ulBox).eq(num).find('li').outerWidth(true));
                    $this.find(ulBox).eq(num).width(divW);
                }
            });
        }
    });
    ...
    function isCreatePoint(createPoint, $this) {
        if (createPoint) {
            $this.find('ul').each(function (index) {
                var pointList = "";
                $this.find('ul').eq(index).find('li').each(function () {
                    pointList += "<li class='ytl_pointList'></li>";
                });
                $this.find('ul').eq(index).parent().find('.ytl_pointLists').html(pointList);
                $this.find('ul').eq(index).parent().find(".ytl_pointLists>li").eq(0).addClass("ytl_pointActive");
            });
        }
        else {
