    ...
    $.fn.extend({
        ...
        "lilyWindow": function (options) {
            ...
            return this.each(function () {
                ...
                function diagonalStrips(ids,id) {
                    ...
                    switch (ids) {
                        case 1:
                            ...
                            initialize();
                            s=$this.find(".ytl_pointActive").index();
                ...
                /*清除div节点*/
                function clearDiv() {
                    var imgCon = $this.find(ulLiBox).eq(s-1).find('img').attr('src');
                    $this.find(ulLiBox).eq(s-1).html('');
                    $this.find(ulLiBox).eq(s-1).append("<img src=" + imgCon + " alt=''>");
                }
                /*创建div节点*/
                function createDiv(ids) {
                    switch (ids) {
                        //创建水平div
                        case 1:
                            var imgCon = $this.find(ulLiBox).eq(s).find('img').attr('src');
                            $this.find(ulLiBox).eq(s).html('');
                            for (var i = 0; i < opts.divLevelNum; i++) {
                                $this.find(ulLiBox).eq(s).append("<div class='ytl_imgActive'><img src=" + imgCon + " alt=''></div>");
                                $this.find(ulLiBox).eq(s).find('.ytl_imgActive').eq(i).css({
                                    'position': 'absolute',
                                    'overflow': 'hidden'
                                });
                                $this.find(ulLiBox).eq(s).find('.ytl_imgActive').eq(i).find('img').css({
                                    'width': $this.width(),
                                    'height': $this.height(),
                                    'display': 'block'
                                });
                                ...
                                $this.find(ulLiBox).eq(s).find('.ytl_imgActive').eq(i).find('img').css({
                                    'margin-top': -parseInt($this.find(ulLiBox).eq(s).find('.ytl_imgActive').eq(i).css('top'))
                                });
                            }
                            break;
                        //创建垂直div
                        case 2:
                            ...
                            for (var i = 0; i < opts.divVerticalNum; i++) {
                                ...
                                $this.find(ulLiBox).eq(s).find('.ytl_imgActive').eq(i).find('img').css({
                                    'margin-left': -parseInt($this.find(ulLiBox).eq(s).find('.ytl_imgActive').eq(i).css('left'))
                                });
                ...
                function pointClickHover() {
                    if (opts.pointClickHover=='click') {
                        ...
                        $this.find(".ytl_pointLists>li").on("click",function () {
                            ...
                            var imgCon = $this.find(ulLiBox).eq(afterIndex).find('img').attr('src');
                            $this.find(ulLiBox).eq(afterIndex).html('');
                            $this.find(ulLiBox).eq(afterIndex).append("<img src=" + imgCon + " alt=''>");

                            var imgCon = $this.find(ulLiBox).eq(beforeIndex).find('img').attr('src');
                            $this.find(ulLiBox).eq(beforeIndex).html('');
                            for (var i = 0; i < opts.divLevelNum; i++) {
                                $this.find(ulLiBox).eq(beforeIndex).append("<div class='ytl_imgActive'><img src=" + imgCon + " alt=''></div>");
                                $this.find(ulLiBox).eq(beforeIndex).find('.ytl_imgActive').eq(i).css({
                                    'position': 'absolute',
                                    'overflow': 'hidden'
                                });
                                $this.find(ulLiBox).eq(beforeIndex).find('.ytl_imgActive').eq(i).find('img').css({
                                    'width': $this.width(),
                                    'height': $this.height(),
                                    'display': 'block'
                                });
                                ...
                                $this.find(ulLiBox).eq(beforeIndex).find('.ytl_imgActive').eq(i).find('img').css({
                                    'margin-top': -parseInt($this.find(ulLiBox).eq(beforeIndex).find('.ytl_imgActive').eq(i).css('top'))
                                });
                            }
                            for(var i=1;i<=opts.divLevelNum;i++){
                                var left=$this.width() + i * $this.find(ulLiBox).outerHeight()/opts.divLevelNum;
                                $this.find(ulLiBox).eq(beforeIndex).find('.ytl_imgActive').eq(i-1).animate({'left':left},opts.timer);
                            }
                        });
                    }
                }
